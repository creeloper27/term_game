...
void player_add(){
    ...
    player[player_number-1].ascii=63;
    player[player_number-1].asciidead=63;
    player[player_number-1].h=-1;
    player[player_number-1].b=-1;
    player[player_number-1].ascii_projectile=63;
    player[player_number-1].fire='?';
    player[player_number-1].top='?';
    player[player_number-1].bottom='?';
    player[player_number-1].right='?';
    player[player_number-1].left='?';
}
    ...
    while(r=create_menu("menu.txt",66,31,4,176,178,177,'<','>')){
        ...
        case 3:
            while(r=create_menu("menu_options.txt",66,31,2,176,178,177,'<','>')){
                switch(r){
                case 1:
                    while(r=create_menu("menu_players.txt",66,31,4,176,178,177,'<','>')){
                        switch(r){
                        case 1:
                            //player_edit(selected_player);
                            break;
                        case 2:
                            player_add();
                            break;
                        case 3:
                            player_remove(selected_player);
                            break;
                        }
                    }
                    break;
...
int main(int argc, char *argv[]){
    ...
    while(exit==0){
        ...
        // ????????? ci metto il nome del player seezionato
        // !!!! ci metto il numero di players
...
void Render(int debug,int fps,int fps_time,int delay, char world[HEIGHT][WIDTH], entity_other other[], int other_number, entity_player player[], int player_number, entity_projectile projectile[], int projectile_number){
    ...
    if(debug){
        ...
        sprintf(temp,"\nP0 K=%d D=%d d_t=%d H=%d\tP1 K=%d D=%d d_t=%d H=%d\n",player[0].k,player[0].d,player[0].dead_time,player[0].health,player[1].k,player[1].d,player[1].dead_time,player[1].health);
        atp(&cont,toPrint,temp);
        }
